...
DIR = "C:/Users/JeremyLin/Desktop/PycharmProjects/bert_ocr/data/train/clothes"
    ...
    def save_image(self, rsp_data, word):
        ...
        for image_info in rsp_data['data']:
            try:
                ...
                # 保存图片
                # filepath = '%s/%s/%s' % (DIR, word, str(self.__counter) + str(suffix)) # 保存到指定位置，按文件夹分开
                filepath = '%s/%s' % (DIR, word+str(self.__counter) + str(suffix)) # 保存到同一个指定文件夹
                # filepath = './%s/%s' % (word, str(self.__counter) + str(suffix)) # 保存到当前project
                ...
                print(word + "图片+1,已有" + str(self.__counter) + "张图片")
                self.__counter += 1
        return
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        crawler.start(args.word, args.total_page, args.start_page, args.per_page)  # 抓取关键词为 “安踏”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
    else:
        # 如果不指定参数，那么程序会按照下面进行执行
        crawler = Crawler(0.05)  # 抓取延迟为 0.05

        data = []
        f = open("clothes.txt", "r", encoding="utf-8")  # 设置文件对象
        #预处理数据
        for line in f:
            line = line[:-1] # 删除末尾回车
            line = line + 'logo' # 加上logo关键字，更精确的爬取到图片
            data.append(line)
        print(data)
        for i in data:
            crawler.start(i, 1, 1, 10)  # 抓取关键词为 “xxxx”，总数为 1 页，开始页码为 1，每页10张（即总共 1*10=10 张）